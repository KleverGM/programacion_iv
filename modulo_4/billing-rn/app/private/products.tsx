...
import type {
  Category,
  Product,
} from "../../src/features/catalog/domain/entities";
...
export default function ProductsScreen() {
  ...
  const categoryLabel = useMemo(() => {
    ...
    return (
      categories.find((c: Category) => c.id === form.category)?.name ??
      "Sin categoría"
    );
  ...
  const nextCategoryId = (current: number | null) => {
    ...
    const idx = categories.findIndex((c: Category) => c.id === current);
    const next =
    ...
    <View style={{ flex: 1, backgroundColor: "#0d1117" }}>
      ...
      <View style={{ flex: 1, padding: 16 }}>
        ...
        {loading ? (
          ...
          >
            <ActivityIndicator />
            <Text style={{ color: "#8b949e", marginTop: 8 }}>Cargando...</Text>
          </View>
        ) : error ? (
          <Text style={{ color: "#f85149", fontWeight: "900" }}>{error}</Text>
        ) : (
          <FlatList
            data={products}
